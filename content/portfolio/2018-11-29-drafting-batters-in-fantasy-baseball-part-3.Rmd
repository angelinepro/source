...
description: "I never trusted WAR"
---
...
In the last two posts, I wrote about my draft strategy using projected WAR, and explored fantasy talent by defensive position. I discovered that [WAR](https://www.fangraphs.com/library/misc/war/) (Wins Above Replacement) was not a great way to select players given my league's scoring categories. In the absence of a summary statistic to guide my decisions, I looked at all of the scoring categories. I was pretty frazzled by the end of the draft.
...
In doing research for writing this blog post, I found this [article](https://fantasysixpack.net/creating-fantasy-baseball-player-rater/) on setting up my own rankings based on my league scoring categories, using z-scores. It's a simple statistic that identifies who is above the mean, and by how much. This is so simple, I was kicking myself for using it on draft day. Here's how it works.
...
Forgive the digression again, but I'm still beating myself up about Buster Posey. 
...
Going back to the question about positional talent, would I have had a different pool of talent to choose from if I'd looked at z-scores instead of WAR?
...
In addition to Gary Sanchez, who I identified earlier, Gattis would have also been a good pick based on his projected homeruns and runs batted in. His WAR is quite low, which is why he didn't end up on my radar before. Let's look at shortstops, just for completeness.
...
Let's look at my second base pick, since prior analysis determined that it was also a position with scarce offensive talent.
...
Alright, so now that I'm satisfied knowing that using z-scores would have changed my picks for catcher and shortstop, I'm going to look at z-scores for the rest of my draft picks. 

####My draft picks
Clearly, using projected z-scores instead of projected WAR would have resulted in different draft picks. Let's look at the z-scores for the players I did end up drafting. I added in information on draft order as well. For reference, I named my team "Dropped Third Strike", after the [obscure baseball rule](https://sabr.org/research/dropped-third-strike-life-and-times-rule), so I've shortened it here to DTS for naming purposes.
